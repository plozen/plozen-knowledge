# 이 파일은 07-History 노트를 dry-run 하거나 Knowledge API에 ingest하는 터미널 명령을 제공한다.
# HistoryDocument 목록을 읽고 Knowledge API의 /documents/ingest 엔드포인트로 보낸다.
...
from .history_ingest import collect_history_documents, default_vault_root, parse_date


def build_parser() -> argparse.ArgumentParser:
    # history ingest CLI에서 받을 날짜, agent, dry-run 옵션을 정의한다.
    load_dotenv()
    parser = argparse.ArgumentParser(description="Ingest Obsidian 07-History notes into PLOZEN Knowledge.")
    ...
    parser.add_argument("--date-from", help="Inclusive history date lower bound, YYYY-MM-DD.")
    parser.add_argument("--date-to", help="Inclusive history date upper bound, YYYY-MM-DD.")
    parser.add_argument("--agent", help="Filter by history filename agent suffix, e.g. nexus or pulse.")
    parser.add_argument("--limit", type=int, help="Maximum number of history files to process.")
...
def main() -> int:
    ...
    documents = collect_history_documents(
        args.vault_root,
